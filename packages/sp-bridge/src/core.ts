...
export class BridgeCore {
  ...
  listTasks(filter: TaskFilter = {}): Record<string, unknown>[] {
    ...
    if (filter.plannedForToday) {
      ...
      tasks = tasks.filter(
        (t) => todayTaskIds.has(t.id as string) || t.dueDay === today,
      );
    }
  ...
  async removeTagFromTask(id: string, tagId: string): Promise<Record<string, unknown>> {
    ...
    return this.updateTask(
      id,
      { tagIds: current.filter((t) => t !== tagId) },
    );
  }

  /** Moves a top-level task to another project (errors on subtasks). */
  async moveTaskToProject(id: string, projectId: string): Promise<Record<string, unknown>> {
    const task = this.getTask(id);
    if (!task) throw err('Task not found', 404);
    if (task.parentId) throw err('Cannot move a subtask between projects', 400);
    if (!this._bucket('PROJECT')[projectId]) throw err(`Unknown projectId: ${projectId}`, 400);
    const op = await this.ops.moveTaskToProject(task, projectId, this.store.nextWriteClock());
    await this.store.submitOps([op]);
  ...
  async createTaskWithSubtasks(
    ...
    for (const title of subTaskTitles) {
      ...
      ops.push(await this.ops.addSubTask(sub, parent.id as string, this.store.nextWriteClock()));
    }
    await this.store.submitOps(ops);
  ...
  async reparentTask(
    ...
    const op = await this.ops.convertToSubTask(taskId, newParentId, this.store.nextWriteClock());
    await this.store.submitOps([op]);
    return this.getTask(taskId) ?? task;
  }
  ...
  async reorderTasks(
    ...
    if (container.projectId) {
      ...
      op = await this.ops.updateProject(container.projectId, { taskIds }, this.store.nextWriteClock());
      ...
      this._assertPermutation(current, taskIds);
      op = await this.ops.updateTask(container.parentId, { subTaskIds: taskIds }, this.store.nextWriteClock());
    } else {
      const today = this._bucket('TAG')[TODAY_TAG_ID];
      current = today && Array.isArray(today.taskIds) ? (today.taskIds as string[]) : [];
      this._assertPermutation(current, taskIds);
      op = await this.ops.updateTag(TODAY_TAG_ID, { taskIds }, this.store.nextWriteClock());
    }
    await this.store.submitOps([op]);
  ...
  async removeTasksFromToday(taskIds: string[]): Promise<{ removed: string[] }> {
    ...
    const op = await this.ops.removeTasksFromTodayTag(taskIds, this.store.nextWriteClock());
    await this.store.submitOps([op]);
  ...
  async deleteTag(id: string): Promise<{ deleted: string }> {
    if (id === TODAY_TAG_ID) throw err('The TODAY tag is virtual and cannot be deleted', 400);
